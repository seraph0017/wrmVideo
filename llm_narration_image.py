...
def regenerate_failed_image(image_path: str) -> bool:
    ...
    try:
        ...
        regenerate_prompt = "基于原图进行改进，一个古代中国女性角色，穿着传统服装，必须是圆领、立领或高领，绝对不能是V领、交领、衽领、y字型领或其他露出脖子以下皮肤的领型，脖子必须完全被服装遮盖不能有任何暴露，后背脖子以下不能有皮肤暴露，胸部不能有皮肤暴露，角色只能有两只手，不能有三只手或更多手臂，画面清晰，细节丰富，宫崎骏动漫风格"
