...
确保字数控制在30-32字，并检查总解说内容字数在1100-1300字之间，自动调用模型API改写不符合要求的内容

使用方法:
python video_scripts/20251124v1/validate_narration.py data/xxx
python video_scripts/20251124v1/validate_narration.py data/xxx --auto-fix
"""
...
# 添加项目根目录到Python路径
# 当前文件路径: video_scripts/20251124v1/validate_narration.py
# 项目根目录: video_scripts/20251124v1/validate_narration.py -> ../../
current_dir = os.path.dirname(os.path.abspath(__file__))
project_root = os.path.dirname(os.path.dirname(current_dir))
if project_root not in sys.path:
    sys.path.insert(0, project_root)

# 尝试导入 Ark，如果失败则设为 None（只在需要 LLM 功能时才会报错）
try:
    from volcenginesdkarkruntime import Ark
except ImportError:
    Ark = None
    print("警告: 未安装 volcenginesdkarkruntime 模块，LLM 自动修复功能将不可用")
...
try:
    ...
    ARK_CONFIG = None
    print("警告: 无法导入配置文件，LLM 自动修复功能将不可用")
...
def rewrite_entire_narration_with_llm(client, all_narrations, max_retries=3):
    """
    使用LLM重写整个narration文件的解说内容，将总字数控制在1100-1300字之间
    ...
    for attempt in range(max_retries):
        if attempt == 0:
            ...
            emphasis = "总字数必须严格控制在1100-1300字之间，这是硬性要求"
        else:
            emphasis = f"总字数必须在1100-1300字之间，当前是第{attempt+1}次尝试，请务必满足字数要求"
...
重写后："""
        ...
        try:
            ...
            if len(rewritten_narrations) == len(all_narrations):
                ...
                if 1100 <= total_chars <= 1300:
                    print(f"  重写成功: 满足1100-1300字要求")
                    return rewritten_narrations
                else:
                    print(f"  字数不符合要求({total_chars}字)，需要在1100-1300字之间，继续重试...")
            else:
...
def validate_narration_file(narration_file_path, client=None, auto_rewrite=False, auto_fix_characters=False, auto_fix_tags=False, auto_fix_structure=False):
    ...
    try:
        ...
        total_valid = 1100 <= total_char_count <= 1300
        total_rewritten = False
        
        # 如果总字数不符合要求且启用自动改写
        if not total_valid and auto_rewrite and client:
            print(f"  总解说内容字数不符合要求({total_char_count}字)，需要在1100-1300字之间，正在重写...")
    ...
    for character_block in character_matches:
        ...
        if modern_clothing_match:
            ...
            if clothing_match:
                ...
                if accessory_match:
                    accessory_text = accessory_match.group(1).strip()
                    if accessory_text and accessory_text != '无其他装饰' and accessory_text != '无':
                        description_parts.append(f"配饰：{accessory_text}")
...
def validate_data_directory(data_dir, auto_rewrite=False, auto_fix_characters=False, auto_fix_tags=False, auto_fix_structure=False):
    ...
    client = None
    if auto_rewrite:
        if Ark is None or ARK_CONFIG is None:
            print(f"警告: LLM 功能不可用（缺少 volcenginesdkarkruntime 模块或配置）")
            print("将继续验证但不进行自动改写")
            auto_rewrite = False
        else:
            try:
    ...
    for chapter_dir in chapter_dirs:
        ...
        print(f"总解说字数: {total_status} {total_narration['total_char_count']}字{total_rewrite_info} (要求: 1100-1300字)")
...
def main():
    ...
    if len(sys.argv) == 2 and not sys.argv[1].startswith('-'):
        # 旧格式: python validate_narration.py data/xxx - 只生成 scene 文件，不启用 LLM 修复
        data_dir = sys.argv[1]
        auto_rewrite = False  # 默认不启用（需要 LLM）
        auto_fix_characters = False  # 默认不启用（需要 LLM）
        auto_fix_tags = False  # 默认不启用
        auto_fix_structure = False  # 默认不启用（需要 LLM）
        print("提示: 将生成 scene 文件（不启用 LLM 自动修复功能）")
    else:
...
3. 如果缺少<解说内容>，根据<图片prompt>生成约55字的解说
4. 保持原有内容不变，只补充缺失的标签
5. 只返回修复后的分镜内容，不要解释
